...
#include <Pixel\Subtraction.h>
#include <Pixel\Image.h>

namespace Pixel
{
	Subtraction::Subtraction(Image* pImage)
	{
		m_pBaseImage = NULL;
		SAFE_ASSIGN(m_pBaseImage, pImage);
	}

	void Subtraction::setBaseImage(Image* pImage)
	{
		SAFE_ASSIGN(m_pBaseImage, pImage);
	}

	Image* Subtraction::getBaseImage() const
	{
		return m_pBaseImage;
	}

	Image* Subtraction::process(Image* pInImage)
	{
