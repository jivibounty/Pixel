...
#include <Pixel\Difference.h>
#include <Pixel\Image.h>
#include <Math.h>

namespace Pixel
{
	Difference::Difference(Image* pImage)
	{
		m_pBaseImage = NULL;
		SAFE_ASSIGN(m_pBaseImage, pImage);
	}

	void Difference::setBaseImage(Image* pImage)
	{
		SAFE_ASSIGN(m_pBaseImage, pImage);
	}

	Image* Difference::getBaseImage() const
	{
		return m_pBaseImage;
	}

	Image* Difference::process(Image* pInImage)
	{
